...
//!
//! # What MPP does, and where this module fits
//!
//! For a representative query like
//! `SELECT COUNT(*) FROM f JOIN p ON f.id = p.fileId WHERE f.content ||| 'Section'`
//! — a scalar aggregate over a join — the MPP win comes from
//! **hash-partitioning the join inputs**, not from partitioning the aggregate
//! output. Per worker, the physical plan looks like:
//!
//! ```text
...
//!
//! [`wrap_with_mpp_shuffle`] is the single primitive that wraps any
//! `ExecutionPlan` with an [`MppShuffleExec`] declaring
//! `Partitioning::Hash(keys, N)` natively (or `UnknownPartitioning(1)` for
//! the scalar-final gather). The exec consumes both halves of the mesh edge
...
//! peer-shipped rows from `shm_mq`) on the participant's `drive_partition`.
//! Other partitions return an empty stream.
//!
//! The output stream emits exactly the rows of `inner` that hash to this
//! participant — some from local computation, some from peers — merged into
//! a single Arrow IPC-compatible `SendableRecordBatchStream`.
//!
...
//! wrapped child. Each wrap consumes one directed mesh of shm_mqs (one
//! `ShuffleWiring` + one `DrainHandle`), so a binary hash-partitioned join
...
///
/// Shape-aligned with `datafusion-distributed::NetworkShuffleExec`.
///
