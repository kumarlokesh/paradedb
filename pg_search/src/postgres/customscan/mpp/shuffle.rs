...
//!
//! [`MppShuffleExec`] is a single `ExecutionPlan` that declares
//! `Partitioning::Hash(keys, N)` natively (or `UnknownPartitioning(1)` for
//! the scalar-final gather), consumes both halves of the mesh edge
//! internally, and merges the producer-side stream (drives upstream,
//! hash-routes via [`RowPartitioner`], ships to peers through
//! [`MppSender`]) with the consumer-side stream (drains peer-shipped rows
//! from `shm_mq`) on the participant's `drive_partition`. Other partitions
//! return an empty stream. The pure-function primitives ([`RowPartitioner`],
//! [`HashPartitioner`], [`split_batch_by_partition`]) live alongside the
//! operator so a reader can step from "row → destination" through
//! "Stream poll body" without crossing files.
//!
//! # Why a separate operator instead of `datafusion_distributed::NetworkShuffleExec`
//!
//! The fork's [`NetworkShuffleExec`] has the same plan-shape role we need
//! (a `NetworkBoundary` that exposes hash partitioning to DataFusion's
//! optimizer) but is hardwired to Arrow Flight as the transport: its
//! `execute()` dials a URL-addressed worker via Tonic/gRPC, returns a
//! `FlightRecordBatchStream`, and demuxes per-partition `tokio::sync::mpsc`
//! channels backed by a memory-reservation budget.
//!
//! ParadeDB MPP runs entirely inside one Postgres backend's parallel-worker
//! fleet, talking through DSM `shm_mq` ring buffers. There are no URLs, no
//! HTTP server, no async pull from a network stream — the transport is
//! `shm_mq_send` / `shm_mq_receive` with PG-style cancellation, fixed-size
//! queues allocated once at DSM init, and a dedicated drain thread per
//! participant that decouples consumer-side backpressure from producer-side
//! (deadlock-avoidance — see `transport::DrainBuffer`). None of those map
//! onto the Flight model cleanly.
//!
//! The fork's `BoundaryFactory` extension point exists exactly for this
//! case. Our [`super::walker::MppBoundaryFactory`] returns `MppShuffleExec`
//! instead of `NetworkShuffleExec`, the walker calls our factory, the fork
//! emits the right boundary, our `NetworkBoundary` impl on `MppShuffleExec`
//! carries it through. **Same plan-shape vocabulary
//! ([`NetworkBoundary`](datafusion_distributed::NetworkBoundary),
//! [`Stage`](datafusion_distributed::Stage), `BoundaryFactory`), different
//! transport implementation.**
//!
//! Making `NetworkShuffleExec` itself transport-agnostic (a `WorkerTransport`
//! trait abstracting URL resolution, the demux task, the memory-reservation
//! model, cancellation hooks) is the kind of refactor that probably belongs
//! upstream of our fork in `datafusion-contrib/datafusion-distributed`, not
//! squeezed into a downstream PR. Until that lands, this module owns the
//! shm_mq-specific concerns.
//!
//! # Primitives
//!
//! - [`RowPartitioner`] is the trait `MppShuffleExec`'s producer loop calls
//!   per batch to decide which destination participant each row belongs to.
...
//!   same input rows places them on the same participant. Downstream
//!   `HashJoinExec` and `AggregateExec` intentionally use *different* seeds
//!   internally (see their `HASH_JOIN_SEED` / `AGGREGATION_HASH_SEED`
//!   constants) — that keeps routing distribution and internal hash-table
//!   distribution decoupled, and has no bearing on our shuffle correctness.
//! - [`FixedTargetPartitioner`] is the gather variant: routes every row to
//!   one fixed target. Used by `MppBoundaryFactory::coalesce` for the
//!   scalar-final gather case.
...
//!   submodule and is reused by `plan_build.rs`'s tests via the same path.
...
//!   preserving row order within each destination. The producer loop calls
//!   this once per input batch, then feeds each sub-batch to its corresponding
//!   [`MppSender`] (peers) or to the cooperative drain (self).
//!
...
/// The walker emits one `MppShuffleExec` per cut.
///
/// # Execution model
///
