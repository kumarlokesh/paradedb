...
///   the Partial, which the boundary factory rewrites to a
///   `FixedTargetPartitioner(0)` (final-gather to leader).
///
...
/// Body invoked once per emitted boundary by the
/// [`PooledBoundaryFactory`] in [`distribute_plan_generic`]. Wires a
/// pre-allocated mesh into an `MppShuffleExec` tagged for the given shape
/// and cut index.
///
/// Routing per [`BoundaryKind`]:
///
/// * [`BoundaryKind::Shuffle`] — extract `Hash(keys)` from the
///   `RepartitionExec(Hash)` child synthesized by [`insert_mpp_cuts`],
///   build a [`HashPartitioner`], drive on the local participant.
/// * [`BoundaryKind::Coalesce`] — fixed-target gather to participant 0
///   ([`FixedTargetPartitioner`]); no hash keys to declare.
/// * [`BoundaryKind::Broadcast`] — never emitted by ParadeDB; surface a
///   planner error rather than silently no-op.
///
/// ParadeDB's `u64` `query_id` round-trips as the low 64 bits of the
/// fork's UUID (`Uuid::from_u128(query_id as u128)`). The wire-format
/// [`MppTaskKey`] is reconstructed from the local primitives below, not
/// from the Stage, so the conversion stays at this single seam.
...
/// [`Column`](datafusion::physical_expr::expressions::Column) — only
/// column-ref join keys are supported.
/// Tuple returned by [`shuffle_keys_from_repartition`]: the production
/// [`HashPartitioner`], the underlying `ExecutionPlan` whose rows the shuffle
/// consumes, and the original hash-key `PhysicalExpr` list — the latter is
/// forwarded into [`MppShuffleExec`] so the wrapped output declares
...
// walker (`insert_mpp_cuts` → `annotate_plan` → `_distribute_plan`) with
// explicit shape-specific pre-passes (`prepare_for_mpp`) and post-passes
// (`finalize_for_mpp`). ParadeDB-specific obligations that don't fit DF-D's
// generic emit model (probe-side dynamic filter strip, HashJoinExec rebuild
// with `PartitionMode::Partitioned`, CoalesceBatchesExec(65_536), leader-only
// FinalPartitioned, VisibilityCtidResolverRule re-run) live in the pre/post
// passes — the walker body stays verbatim-DF-D.
//
// The dispatcher above (`distribute_plan`) routes all three live shapes
// through this pipeline (`JoinOnly`, `ScalarAggOnBinaryJoin`,
...
/// Glue together the generic walker (`insert_mpp_cuts` → `annotate_plan_sync`
/// → `distribute_annotated_plan`) with the shape-specific pre/post passes.
/// Called by [`distribute_plan`] for shapes whose post-passes have been
/// ported.
fn distribute_plan_generic(
    ...
    // Drain the pre-allocated meshes into the fork's
    // [`PooledBoundaryFactory`]. The closure dispatches per [`BoundaryKind`]
    // and wires each mesh into an `MppShuffleExec` tagged for its bottom-up
    // cut index. The fork's `_distribute_plan` increments `stage_id` once
    // per emitted boundary bottom-up, which is exactly the cut index our
    // `tag_for_cut` and `Stage::new_unaddressed` consume; starting from 0
    // keeps the tag mapping identical to the legacy walker.
...
fn finalize_groupby_agg(
    ...
    // the participant's complete output. The post-agg [`MppShuffleExec`]
    // declares `Partitioning::Hash(group_keys, N)` — without this gather,
    // only partition 0's groups would surface to PG and the other N-1
    // participants' contributions would be dropped.
    Ok(Arc::new(CoalescePartitionsExec::new(Arc::new(final_agg))))
}

/// Walk down `plan`, find the first [`MppShuffleExec`], and wrap its single
/// child in `CoalesceBatchesExec(65_536)`. Used by [`finalize_groupby_agg`]
/// to coalesce the post-Partial payload before the post-agg shuffle.
///
/// The `with_new_children` chain rebuilds the ancestor nodes; siblings of
/// the `MppShuffleExec` are preserved by-reference.
